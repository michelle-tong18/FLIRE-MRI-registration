...
writeFullCorrectedVolumes(const InputParams& p, fvol* forwardMri, fvol* reverseMri,
  ...
  fvol* forwardMriNoJ ( createRescaledMri(width, height, depthOrig, xsize, ysize, zsize, forwardMri, fillDataNO) ); // Corrected but with no Jacobian intensity scaling
  fvol* reverseMriNoJ ( createRescaledMri(width, height, depthOrig, xsize, ysize, zsize, reverseMri, fillDataNO) ); 
  fvol* forwardJMri   ( createRescaledMri(width, height, depthOrig, xsize, ysize, zsize, forwardMri, fillDataNO) ); // The Jacobian field
  fvol* reverseJMri   ( createRescaledMri(width, height, depthOrig, xsize, ysize, zsize, forwardMri, fillDataNO) );
  ...
  for(int x = 0; x < width; ++x)
	...
	float fJ ( fabs(widthb1/widtha) );
	float rJ ( fabs(widthb2/widtha) );
	...
	fVolSetVal(forwardMriNoJ, y, x, za, fvalNoJ);
	fVolSetVal(reverseMriNoJ, y, x, za, rvalNoJ);
	fVolSetVal(forwardJMri,   y, x, za, fJ);
	fVolSetVal(reverseJMri,   y, x, za, rJ);
      }
  ...
  string forwardNoJFile       ( outDir + "fB0uwNoJ.mgz" );
  string reverseNoJFile       ( outDir + "rB0uwNoJ.mgz" );
  string forwardJFile         ( outDir + "fJ.mgz" );
  string reverseJFile         ( outDir + "rJ.mgz" );
  ...
  writeImage(forwardNoJFile,       forwardMriNoJ);
  writeImage(reverseNoJFile,       reverseMriNoJ);
  writeImage(forwardJFile,         forwardJMri);
  writeImage(reverseJFile,         reverseJMri);
  ...
  fVolDelete(difMriC);
  fVolDelete(dispyMri);  
  fVolDelete(forwardMriNoJ);
  fVolDelete(reverseMriNoJ);
  fVolDelete(forwardJMri);
  fVolDelete(reverseJMri);
