...
computeDeformationField(fvol* forwardMri, fvol* reverseMri, const InputParams& p) {
  ...
  if( p.getRestart() )
    loadDispy(p.getDisplacementFieldInFileName(), numVox, voxStep, p.getNvoxNewZbdry(), p, ddub);
  else
  ...
  const float l1Min  ( 0.0 );        // <--- Was using this!
//const float l1Min  ( l1Max/8 );
  const float l2Max  ( lambda2 );
  const float l2Min  ( 1.0e3 );      // <--- Was using this!
//const float l2Min  ( l2Max/8 );
  const float l2PMax ( lambda2P );
  const float l2PMin ( 1.0e3 );      // <--- Was using this!
//const float l2PMin ( l2PMax/8 );
  ...
  for(int kernelWidth = kernelWidthMax; kernelWidth > -kernelWidthStep; ) {
    ...
    lambda1  = l1Max ; clog << "lambda1  = "<<lambda1 <<endl;
    lambda2  = l2Max ; clog << "lambda2  = "<<lambda2 <<endl;
    lambda2P = l2PMax; clog << "lambda2P = "<<lambda2P<<endl;
