...
class InputParams {
 ...
 public:
  ...
  const bool&        getResample()           const { return resample; }
  
  const float&       getXs()                 const { return xs; }
  const float&       getYs()                 const { return ys; }
  const float&       getZs()                 const { return zs; }  
  
  const int&         getSmoothDfieldKernelWidth() const { return smoothDfieldKernelWidth; }  
