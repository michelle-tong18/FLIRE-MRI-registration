...
InputParams::readParameters(int argc, char** argv) {
  ...
  bool resampleFlag (false);// If set, commandline value will override value in file, if provided.
  pos = find(commandLine.begin(), commandLine.end(), "-resample");
  if(pos != commandLine.end()) {
    resample = true;        // Default set to false
    ++pos;                 // don't forget to iterate on...!
    resampleFlag = true;
  }
  
  bool xsFlag (false);
  pos = find(commandLine.begin(), commandLine.end(), "-xs");
  if(pos != commandLine.end()) {
    if(++pos != commandLine.end()) {
  ...
  bool ysFlag (false);
  pos = find(commandLine.begin(), commandLine.end(), "-ys");
  if(pos != commandLine.end()) {
    if(++pos != commandLine.end()) {
      stringstream ssn(*(pos)); // turn the string into a float
      ssn >> ys;
  ...
  bool zsFlag (false);
  pos = find(commandLine.begin(), commandLine.end(), "-zs");
  if(pos != commandLine.end()) {
    if(++pos != commandLine.end()) {
      stringstream ssn(*(pos)); // turn the string into a float
      ssn >> zs;
  ...
  bool smoothDfieldKernelWidthFlag (false);
  pos = find(commandLine.begin(), commandLine.end(), "-smoothDfieldKernelWidth");
  if(pos != commandLine.end()) {
    if(++pos != commandLine.end()) {
      stringstream ssn(*(pos)); // turn the string into an int
      ssn >> smoothDfieldKernelWidth;
    }
    smoothDfieldKernelWidthFlag = true;
  }
  ...
  while(getline(fParamsIn, s))  {
    ...
    else if(!resampleFlag && v[0] == "resample")  {                // v[2] will be "0" or "1". Translate to true or false.
      stringstream ssn(v[2]);                                    // stringstream --> bool
      ssn >> resample;  count++;                                  // Default set to false
    }
    else if(!xsFlag && v[0] == "xs")  {
      stringstream ssn(v[2]);              // turn the string into a float
      ssn >> xs;
      count++;
    }
    else if(!ysFlag && v[0] == "ys")  {
      stringstream ssn(v[2]);              // turn the string into a float
      ssn >> ys;
      count++;
    }
    else if(!zsFlag && v[0] == "zs")  {
      stringstream ssn(v[2]);              // turn the string into a float
      ssn >> zs;
      count++;
    }
    else if(!smoothDfieldKernelWidthFlag && v[0] == "smoothDfieldKernelWidth")  {
      stringstream ssn(v[2]);              // turn the string into an int
      ssn >> smoothDfieldKernelWidth;
      count++;
    }
...
InputParams::print(ostream& os) const {
  ...
  os << "resample                     = " << resample                     << '\n';
  
  os << "xs                           = " << xs                           << '\n';
  os << "ys                           = " << ys                           << '\n';
  os << "zs                           = " << zs                           << '\n';
  
  os << "smoothDfieldKernelWidth      = " << smoothDfieldKernelWidth      << '\n';
